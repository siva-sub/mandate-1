...
Kaggle distribution: `kaggle datasets download -d sivasub987/mandate-1-safr-data --unzip -p data-download`. Nested dataset/typed/provenance directories are packaged in an explicit `data-bundle.zip`; extract it before using the standard-library example.
